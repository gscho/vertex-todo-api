...
import io.vertx.todolist.entity.Todo;
import io.vertx.todolist.routes.Routes;
...
public class TodoVerticle extends AbstractVerticle {
	...
	public void start(Future<Void> future) {
		...
		router.get(Routes.API_GET).handler(this::handleGetTodo);
		router.get(Routes.API_LIST_ALL).handler(this::handleGetAll);
		router.post(Routes.API_CREATE).handler(this::handleCreateTodo);
		router.put(Routes.API_UPDATE).handler(this::handleUpdateTodo);
		router.delete(Routes.API_DELETE).handler(this::handleDeleteOne);
		router.delete(Routes.API_DELETE_ALL).handler(this::handleDeleteAll);
	...
	private void initData() {
		...
		redis.hset(REDIS_TODO_KEY, "24", Json.encodePrettily( // test connection
	...
	private void handleGetTodo(RoutingContext context) {
		...
		if (todoID == null)
			...
			redis.hget(REDIS_TODO_KEY, todoID, x -> { // (3)
				if (x.succeeded()) {
	...
	private void handleGetAll(RoutingContext context) {
		redis.hvals(REDIS_TODO_KEY, res -> { // (1)
			if (res.succeeded()) {
				String encoded = Json.encodePrettily(res.result().stream() // (2)
						.map(x -> {
							try {
								return new Todo((String) x);
							} catch (IOException e) {
			...
			redis.hget(REDIS_TODO_KEY, todoID, x -> { // (3)
				if (x.succeeded()) {
					...
					if (result == null)
						...
						Todo oldTodo = null;
						try {
							oldTodo = new Todo(result);
						} catch (IOException e) {
							sendError(400, context.response());
						}
						String response = Json.encodePrettily(oldTodo.merge(newTodo)); // (5)
						redis.hset(REDIS_TODO_KEY, todoID, response, res -> { // (6)
							if (res.succeeded()) {
